    ...
    while (1)
    {
        ...
        switch (msg_cmd)
        {
            ...
            case IPC_CMD_SET_TEMPERATURE_DATA:
                ...
                printf("Current T: %u, Target T: %u, RS: %u\n", temp_data.environment.current_temp,
            ...
            case IPC_CMD_SWITCH_TO_BLE:
                ...
                if (conn_id != true)
                {
                    ...
                    /* Wait for MQTT task to disconnect */
                    while (switch_to_ble_mqtt_disconn != false)
                    {
                        vTaskDelay(100);
                        printf("IPC_CMD_SWITCH_TO_BLE Wait switch_to_ble_mqtt_disconn\n");
                    }
                    ble_start_advertising(BTM_BLE_ADVERT_UNDIRECTED_HIGH);
                }
                else
                {
                    printf("Device already connected over BLE\n");
                }
                break;
            ...
            case IPC_CMD_UPDATE_CONN_STATE:
                switch ((device_connection_state_t) msg_val)
                {
                    case DEV_ST_CLOUD_CONNECTION_CANCEL:
                        ...
                        mqtt_task_cmd = HANDLE_MANUAL_DISCONNECTION;
                        xQueueSend(mqtt_task_q, &mqtt_task_cmd, portMAX_DELAY);
                        printf("Rx DEV_ST_CLOUD_CONNECTION_CANCEL.\n");
                        update_comm_interface(CONNECTIVITY_NONE);
                        break;

                    case DEV_ST_WIFI_CONNECTION_CANCEL:
                        printf("Rx DEV_ST_WIFI_CONNECTION_CANCEL.\n");
                        ...
                        if (conn_id != false)
                        {
                            printf("DEV_ST_WIFI_CONNECTION_CANCEL BLE already connected.\n");
                            update_conn_state(DEV_ST_BLE_CONNECTED);
                            handle_connectivity_state(STATE_BLE_CONNECTED);
                        }
                        else
                        {
                            printf("DEV_ST_WIFI_CONNECTION_CANCEL BLE start ADV.\n");
                    ...
                    case DEV_ST_BLE_ADVERTISEMENT_CANCEL:
                        ...
                        else
                        {
                            ...
                            printf("Wifi Credential not available.\n");
                        }
                        update_comm_interface(CONNECTIVITY_NONE);
                        ble_stop_advertising();
                        printf("Rx DEV_ST_BLE_ADVERTISEMENT_CANCEL.\n");
                        break;
...
//                        ble_stop_advertising();
                        printf("Request to switch to WiFi connection\n");
                        break;
            ...
            case IPC_CMD_DEVICE_CONFIG:
                printf("Rx IPC_CMD_DEVICE_CONFIG\n");
