/*******************************************************************************
 * \file mtb_ctp_ft5xx6.c
 * \version 0.5.0
 *
 * \brief
 * Provides implementation of the FT5XX6 CTP driver library.
 *
 ********************************************************************************
 * \copyright
 ...
 * Global Variables
 *******************************************************************************/
static mtb_ctp_ft5xx6_config_t *ctp_config = NULL;

/*******************************************************************************
 * Function Name: mtb_ctp_ft5xx6_i2c_controller_write
 ********************************************************************************
 *
 * Performs I2C controller write to the FT5XX6 CTP target.
 *
 * \param write_buffer
 ...
 *
 *******************************************************************************/
static cy_en_scb_i2c_status_t mtb_ctp_ft5xx6_i2c_controller_write ( uint8_t *write_buffer, uint32_t buff_size )
{
    ...
    transfer_config.slaveAddress = FT5XX6_I2C_ADDR;
...
/*******************************************************************************
 * Function Name: mtb_ctp_ft5xx6_i2c_controller_read
 ********************************************************************************
 *
 * Performs I2C controller read from the FT5XX6 CTP target.
 *
 * \param read_buffer
 ...
 *
 *******************************************************************************/
static cy_en_scb_i2c_status_t mtb_ctp_ft5xx6_i2c_controller_read ( uint8_t *read_buffer, uint32_t buff_size )
{
    cy_stc_scb_i2c_master_xfer_config_t transfer_config;
    cy_en_scb_i2c_status_t i2c_status = CY_SCB_I2C_SUCCESS;
    uint8_t retry_count = RETRY_COUNT;
...
/*******************************************************************************
 * Function Name: mtb_ctp_ft5xx6_is_alive
 ********************************************************************************
 *
 * This function checks whether FT5XX6 IC is alive or not by reading the chip
 * ID and firmware details of it.
 *
 * \param void
 *
 * \return cy_en_scb_i2c_status_t
 * FT5XX6 alive status in terms of I2C operation result
 *
 *******************************************************************************/
static cy_en_scb_i2c_status_t mtb_ctp_ft5xx6_is_alive ( void )
{
    cy_en_scb_i2c_status_t status = CY_SCB_I2C_SUCCESS;
    uint8_t cmd = (uint8_t) FT5XX6_REG_LIB_CHIP_VENDOR_ID;
    uint8_t chipID[1] = { (uint8_t)RESET_VAL };

    /* Send I2C packet with command to the target */
    status = mtb_ctp_ft5xx6_i2c_controller_write(&cmd, sizeof(cmd));

    if ( CY_SCB_I2C_SUCCESS == status )
    {
        status = mtb_ctp_ft5xx6_i2c_controller_read(chipID, 1);
        if ( CY_SCB_I2C_SUCCESS == status )
        {
            if(*chipID != FT5446_VENDOR_ID)
            {
...
/*******************************************************************************
 * Function Name: mtb_ctp_ft5xx6_read_raw_touch_data
 ********************************************************************************
 *
 ...
 *
 *******************************************************************************/
static cy_en_scb_i2c_status_t mtb_ctp_ft5xx6_read_raw_touch_data ( uint32_t *touch_data)
{
    cy_en_scb_i2c_status_t i2c_status = CY_SCB_I2C_SUCCESS;
    uint8_t get_touch_cmd = (uint8_t) FT5XX6_REG_TOUCH1_XH;
    uint8_t i2cRx[4] = { (uint8_t)RESET_VAL };

    CY_ASSERT(NULL != touch_data);

    i2c_status = mtb_ctp_ft5xx6_i2c_controller_write(&get_touch_cmd, sizeof(get_touch_cmd));
    if ( CY_SCB_I2C_SUCCESS == i2c_status )
    {
        i2c_status = mtb_ctp_ft5xx6_i2c_controller_read(i2cRx, 4);
...
/*******************************************************************************
 * Function Name: mtb_ctp_ft5xx6_irq_handler
 ********************************************************************************
 *
 ...
 *
 *******************************************************************************/
__WEAK void mtb_ctp_ft5xx6_irq_handler ( void )
{
...
/*******************************************************************************
 * Function Name: mtb_ctp_ft5xx6_init
 ********************************************************************************
 *
 * Performs FT5XX6 touch controller driver initialization using I2C interface.
 *
 * \param ctp_ft5xx6_config
 * Pointer to the FT5XX6 touch controller configuration structure.
 *
 * \return cy_rslt_t
 * Initialization status. CY_RSLT_SUCCESS on success else CY_RSLT_FT5XX6_DEV_ERR,
 * I2C error(s).
 *
 * \funcusage
 * \snippet snippet/main.c snippet_mtb_ctp_ft5xx6_init
 *
 *******************************************************************************/
cy_rslt_t mtb_ctp_ft5xx6_init ( mtb_ctp_ft5xx6_config_t *ctp_ft5xx6_config )
{
    ...
    CY_ASSERT(NULL != ctp_ft5xx6_config);

    ctp_config = ctp_ft5xx6_config;
    ctp_config->touch_event = false;

    cy_stc_sysint_t ctp_irq_cfg = { .intrSrc = ctp_config->irq_num, .intrPriority = MTB_CTP_FT5XX6_IRQ_PRIORITY };
...
//      Resetting GPIO here causes DISP to reset which in turns disables the display. If GPIOs are separate than this needs to be enabled
    ...
    /* Perform reset sequence to enable FT5XX6 touch controller */
    ...
    do
    {
        /* Check whether FT5XX6 touch controller is alive or not */
        status = (cy_rslt_t) mtb_ctp_ft5xx6_is_alive();
    ...
    if ( CY_RSLT_SUCCESS == status )
    {
        ...
        Cy_SysInt_Init(&ctp_irq_cfg, &mtb_ctp_ft5xx6_irq_handler);
        NVIC_EnableIRQ(ctp_irq_cfg.intrSrc);
    }
    else
    {
        status = CY_RSLT_FT5XX6_DEV_ERR;
    }
...
/*******************************************************************************
 * Function Name: mtb_ctp_ft5xx6_get_single_touch
 ********************************************************************************
 *
 * This function fetches raw touch coordinates using
 * "mtb_ctp_ft5xx6_read_raw_touch_data" function, applies calibration and
 ...
 * CY_RSLT_FT5XX6_NO_TOUCH, I2C error(s).
 *
 *******************************************************************************/
cy_rslt_t mtb_ctp_ft5xx6_get_single_touch ( int *touch_x, int *touch_y )
{
    ...
    result = (cy_rslt_t) mtb_ctp_ft5xx6_read_raw_touch_data(&touch_data);
...
//
        ...
        {
            result = (cy_rslt_t) CY_RSLT_FT5XX6_NO_TOUCH;
        }
    }
...
/*******************************************************************************
 * Function Name: mtb_ctp_ft5xx6_deinit
 ********************************************************************************
 *
