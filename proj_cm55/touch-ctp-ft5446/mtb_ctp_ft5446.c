/*******************************************************************************
 * \file mtb_ctp_ft5446.c
 * \version 0.5.0
 *
 * \brief
 * Provides implementation of the FT5446 CTP driver library.
 *
 ********************************************************************************
 * \copyright
 ...
 * Global Variables
 *******************************************************************************/
static mtb_ctp_ft5446_config_t *ctp_config = NULL;

/*****************************************************************************
* Function name: mtb_ctp_ft5446_i2c_controller_write
*****************************************************************************
* Summary:
* Performs I2C controller write to the FT5446 CTP target.
*
* Parameters:
*  write_buffer              Pointer to the I2C buffer for data to write to the target.
*  buff_size                 Size of the I2C write buffer.
* Return:
*  cy_en_scb_i2c_status_t    I2C controller write status
*
*****************************************************************************/
static cy_en_scb_i2c_status_t mtb_ctp_ft5446_i2c_controller_write ( uint8_t *write_buffer, uint32_t buff_size )
{
    ...
    transfer_config.slaveAddress = MTB_FT5446_I2C_ADDR;
    ...
    do
    {
        ...
        if ( CY_SCB_I2C_SUCCESS == i2c_status )
        {
            ...
            if ( (!(I2C_CONTROLLER_ERROR_MASK & i2c_status))
...
/*****************************************************************************
* Function name: mtb_ctp_ft5446_i2c_controller_read
*****************************************************************************
* Summary:
* Performs I2C controller read from the FT5446 CTP target.
*
* Parameters:
*  read_buffer               Pointer to the I2C buffer for data to read to the target.
*  buff_size                 Size of the I2C read buffer.
* Return:
*  cy_en_scb_i2c_status_t    I2C controller read status
*
*****************************************************************************/
static cy_en_scb_i2c_status_t mtb_ctp_ft5446_i2c_controller_read ( uint8_t *read_buffer, uint32_t buff_size )
{
    cy_stc_scb_i2c_master_xfer_config_t transfer_config;
    cy_en_scb_i2c_status_t i2c_status = CY_SCB_I2C_SUCCESS;
    uint8_t retry_count = RETRY_COUNT;
...
/*****************************************************************************
* Function name: mtb_ctp_ft5446_is_alive
*****************************************************************************
* Summary:
* This function checks whether FT5446 IC is alive or not by reading the chip
* ID and firmware details of it.
*
* Parameters:                None
* Return:
*  cy_en_scb_i2c_status_t    FT5446 alive status in terms of I2C operation result
*
*****************************************************************************/
static cy_en_scb_i2c_status_t mtb_ctp_ft5446_is_alive ( void )
{
    cy_en_scb_i2c_status_t status = CY_SCB_I2C_SUCCESS;
    uint8_t cmd = (uint8_t) MTB_FT5446_REG_LIB_CHIP_VENDOR_ID;
    uint8_t chipID[1] = { (uint8_t)RESET_VAL };

    /* Send I2C packet with command to the target */
    status = mtb_ctp_ft5446_i2c_controller_write(&cmd, sizeof(cmd));

    if ( CY_SCB_I2C_SUCCESS == status )
    {
        status = mtb_ctp_ft5446_i2c_controller_read(chipID, 1);
        if ( CY_SCB_I2C_SUCCESS == status )
        {
            if(*chipID != MTB_FT5446_VENDOR_ID)
            {
...
/*****************************************************************************
* Function name: mtb_ctp_ft5446_read_raw_touch_data
*****************************************************************************
* Summary:
...
* Return:
*  cy_en_scb_i2c_status_t    Read raw touch data operation status
*
*****************************************************************************/
static cy_en_scb_i2c_status_t mtb_ctp_ft5446_read_raw_touch_data ( uint32_t *touch_data)
{
    cy_en_scb_i2c_status_t i2c_status = CY_SCB_I2C_SUCCESS;
    uint8_t get_touch_cmd = (uint8_t) MTB_FT5446_REG_TOUCH1_XH;
    uint8_t i2cRx[4] = { (uint8_t)RESET_VAL };

    CY_ASSERT(NULL != touch_data);

    i2c_status = mtb_ctp_ft5446_i2c_controller_write(&get_touch_cmd, sizeof(get_touch_cmd));
    if ( CY_SCB_I2C_SUCCESS == i2c_status )
    {
        i2c_status = mtb_ctp_ft5446_i2c_controller_read(i2cRx, 4);
        *touch_data = (i2cRx[0] << 24) + (i2cRx[1] << 16) + (i2cRx[2] << 8) + i2cRx[3];
    }
...
/*****************************************************************************
* Function name: mtb_ctp_ft5446_irq_handler
*****************************************************************************
* Summary:
...
*
*****************************************************************************/
__WEAK void mtb_ctp_ft5446_irq_handler ( void )
{
...
/*****************************************************************************
* Function name: mtb_ctp_ft5446_init
*****************************************************************************
* Summary:
* Performs FT5446 touch controller driver initialization using I2C interface.
*
* Parameters:
*  ctp_ft5446_config         Pointer to the FT5446 touch controller configuration structure.
* Return:
*  cy_rslt_t                 Initialization status. CY_RSLT_SUCCESS on success else 
*                            CY_RSLT_FT5446_DEV_ERR, I2C error(s).
*
*****************************************************************************/
cy_rslt_t mtb_ctp_ft5446_init ( mtb_ctp_ft5446_config_t *ctp_ft5446_config )
{
    ...
    CY_ASSERT(NULL != ctp_ft5446_config);

    ctp_config = ctp_ft5446_config;
    ctp_config->touch_event = false;

    cy_stc_sysint_t ctp_irq_cfg = { .intrSrc = ctp_config->irq_num, .intrPriority = MTB_CTP_FT5446_IRQ_PRIORITY };

// TODO: DISP_RST and TOUCH_RST are same on SmartHMI EVK.
//      Resetting GPIO here causes DISP to reset which in turns disables the display. If GPIOs are separate than this needs to be enabled
#if 0
    ...
    /* Perform reset sequence to enable FT5446 touch controller */
    ...
    do
    {
        /* Check whether FT5446 touch controller is alive or not */
        status = (cy_rslt_t) mtb_ctp_ft5446_is_alive();
    ...
    if ( CY_RSLT_SUCCESS == status )
    {
        ...
        Cy_SysInt_Init(&ctp_irq_cfg, &mtb_ctp_ft5446_irq_handler);
        NVIC_EnableIRQ(ctp_irq_cfg.intrSrc);
    }
    else
    {
        status = CY_RSLT_FT5446_DEV_ERR;
    }
...
/*****************************************************************************
* Function name: mtb_ctp_ft5446_get_single_touch
*****************************************************************************
* Summary:
* This function fetches raw touch coordinates using
* "mtb_ctp_ft5446_read_raw_touch_data" function, applies calibration and
* returns actual x, y touch coordinates.
*
* Parameters:
*  touch_x               Pointer to the X coordinate of touched location.
*  touch_y               Pointer to the Y coordinate of touched location.
* Return:
*  cy_rslt_t             Result of reading single touch data operation, 
*                        CY_RSLT_SUCCESS on success else 
*                        CY_RSLT_FT5446_NO_TOUCH, I2C error(s).
*
*****************************************************************************/
    ...
    if ( CY_RSLT_SUCCESS == result )
    {
        ...
        if ( (last_touch_x != current_touch_x) || (last_touch_y != current_touch_y) )
        {
            ...
            *touch_y = (int)((current_touch_y * MTB_CTP_TOUCH_RESOLUTION_Y) / TP_MAX_VAL_Y);
            ...
            // TODO: Modify as per Orientation
//            switch ( ORIENTATION )
//            {
//                case 90 :
//                    touch_x = MTB_CTP_TOUCH_RESOLUTION_Y - current_touch_y;
//                    touch_y = MTB_CTP_TOUCH_RESOLUTION_X - current_touch_x;
//                    break;
//                case 180 :
//                    touch_x = current_touch_x;
//                    touch_y = MTB_CTP_TOUCH_RESOLUTION_Y - current_touch_y;
//                    break;
//                case 270 :
//                    touch_x = current_touch_y;
//                    touch_y = current_touch_x;
//                    break;
//                default : // 0�
//                    touch_x = MTB_CTP_TOUCH_RESOLUTION_X - current_touch_x;
//                    touch_y = current_touch_y;
//                    break;
//            }
//
...
/*****************************************************************************
* Function name: mtb_ctp_ft5446_deinit
*****************************************************************************
* Summary:
