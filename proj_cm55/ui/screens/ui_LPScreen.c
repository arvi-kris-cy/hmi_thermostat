// This file was generated by SquareLine Studio
// SquareLine Studio version: SquareLine Studio 1.5.4
// LVGL version: 9.1.0
// Project name: Thermostat_HMI

#include "../ui.h"
#include "thermostat_events.h"
#include "app_sensor.h"

lv_obj_t * ui_LPScreen = NULL;
lv_obj_t * ui_LPscreenContainer = NULL;
lv_obj_t * ui_MainTemptextLP = NULL;
lv_obj_t * ui_DegreeLblLP = NULL;
lv_obj_t * ui_TimeHLP = NULL;
lv_obj_t * ui_DateLP = NULL;
lv_obj_t * ui_TimeMLP = NULL;
lv_obj_t * ui_TimeSLP = NULL;
lv_obj_t * ui_LPModelabel = NULL;
lv_obj_t * ui_changescreenLP = NULL;
extern volatile application_state_t app_state;

// event funtions
void ui_event_changescreenLP(lv_event_t * e)
{
    lv_event_code_t event_code = lv_event_get_code(e);

    if(event_code == LV_EVENT_CLICKED) {
         app_state = APP_ST_ACTIVE;
        _ui_screen_change(&ui_ActiveScreen, LV_SCR_LOAD_ANIM_FADE_ON, 10, 0, &ui_ActiveScreen_screen_init);
        start_inactivity_timer();

        /* Set sensor sampling interval to Active state */
        set_sensor_sampling_interval(SENSOR_SAMPLING_INTERVAL_ACTIVE);
    }
}



// build funtions

void ui_LPScreen_screen_init(void)
{
    ui_LPScreen = lv_obj_create(NULL);
    lv_obj_remove_flag(ui_LPScreen, LV_OBJ_FLAG_SCROLLABLE);      /// Flags
    lv_obj_set_style_bg_color(ui_LPScreen, lv_color_hex(0x1A1B1A), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(ui_LPScreen, 255, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_LPscreenContainer = lv_obj_create(ui_LPScreen);
    lv_obj_remove_style_all(ui_LPscreenContainer);
    lv_obj_set_width(ui_LPscreenContainer, 473);
    lv_obj_set_height(ui_LPscreenContainer, 479);
    lv_obj_set_x(ui_LPscreenContainer, -18);
    lv_obj_set_y(ui_LPscreenContainer, -2);
    lv_obj_set_align(ui_LPscreenContainer, LV_ALIGN_CENTER);
    lv_obj_remove_flag(ui_LPscreenContainer, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_SCROLLABLE);      /// Flags

    ui_MainTemptextLP = lv_label_create(ui_LPscreenContainer);
    lv_obj_set_width(ui_MainTemptextLP, LV_SIZE_CONTENT);   /// 1
    lv_obj_set_height(ui_MainTemptextLP, LV_SIZE_CONTENT);    /// 1
    lv_obj_set_x(ui_MainTemptextLP, 0);
    lv_obj_set_y(ui_MainTemptextLP, 0);
    lv_obj_set_align(ui_MainTemptextLP, LV_ALIGN_CENTER);
    lv_label_set_text(ui_MainTemptextLP, "27");
    lv_obj_set_style_text_color(ui_MainTemptextLP, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_opa(ui_MainTemptextLP, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(ui_MainTemptextLP, &ui_font_Roboto80, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_DegreeLblLP = lv_label_create(ui_LPscreenContainer);
    lv_obj_set_width(ui_DegreeLblLP, LV_SIZE_CONTENT);   /// 1
    lv_obj_set_height(ui_DegreeLblLP, LV_SIZE_CONTENT);    /// 1
    lv_obj_set_x(ui_DegreeLblLP, 49);
    lv_obj_set_y(ui_DegreeLblLP, -18);
    lv_obj_set_align(ui_DegreeLblLP, LV_ALIGN_CENTER);
    lv_label_set_text(ui_DegreeLblLP, "°");
    lv_obj_set_style_text_color(ui_DegreeLblLP, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_opa(ui_DegreeLblLP, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(ui_DegreeLblLP, &lv_font_montserrat_40, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_TimeHLP = lv_label_create(ui_LPscreenContainer);
    lv_obj_set_width(ui_TimeHLP, LV_SIZE_CONTENT);   /// 1
    lv_obj_set_height(ui_TimeHLP, LV_SIZE_CONTENT);    /// 1
    lv_obj_set_x(ui_TimeHLP, -59);
    lv_obj_set_y(ui_TimeHLP, -188);
    lv_obj_set_align(ui_TimeHLP, LV_ALIGN_CENTER);
    lv_label_set_text(ui_TimeHLP, "09");
    lv_obj_set_style_text_color(ui_TimeHLP, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_opa(ui_TimeHLP, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_align(ui_TimeHLP, LV_TEXT_ALIGN_LEFT, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(ui_TimeHLP, &lv_font_montserrat_38, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_DateLP = lv_label_create(ui_LPscreenContainer);
    lv_obj_set_width(ui_DateLP, LV_SIZE_CONTENT);   /// 1
    lv_obj_set_height(ui_DateLP, LV_SIZE_CONTENT);    /// 1
    lv_obj_set_x(ui_DateLP, 6);
    lv_obj_set_y(ui_DateLP, -142);
    lv_obj_set_align(ui_DateLP, LV_ALIGN_CENTER);
    lv_label_set_text(ui_DateLP, "Wed 05 Mar");
    lv_obj_set_style_text_color(ui_DateLP, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_opa(ui_DateLP, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(ui_DateLP, &lv_font_montserrat_20, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_TimeMLP = lv_label_create(ui_LPscreenContainer);
    lv_obj_set_width(ui_TimeMLP, LV_SIZE_CONTENT);   /// 1
    lv_obj_set_height(ui_TimeMLP, LV_SIZE_CONTENT);    /// 1
    lv_obj_set_x(ui_TimeMLP, 5);
    lv_obj_set_y(ui_TimeMLP, -188);
    lv_obj_set_align(ui_TimeMLP, LV_ALIGN_CENTER);
    lv_label_set_text(ui_TimeMLP, "10");
    lv_obj_set_style_text_color(ui_TimeMLP, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_opa(ui_TimeMLP, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_align(ui_TimeMLP, LV_TEXT_ALIGN_LEFT, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(ui_TimeMLP, &lv_font_montserrat_38, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_TimeSLP = lv_label_create(ui_LPscreenContainer);
    lv_obj_set_width(ui_TimeSLP, LV_SIZE_CONTENT);   /// 1
    lv_obj_set_height(ui_TimeSLP, LV_SIZE_CONTENT);    /// 1
    lv_obj_set_x(ui_TimeSLP, 68);
    lv_obj_set_y(ui_TimeSLP, -187);
    lv_obj_set_align(ui_TimeSLP, LV_ALIGN_CENTER);
    lv_label_set_text(ui_TimeSLP, "11");
    lv_obj_set_style_text_color(ui_TimeSLP, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_opa(ui_TimeSLP, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_align(ui_TimeSLP, LV_TEXT_ALIGN_LEFT, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(ui_TimeSLP, &lv_font_montserrat_38, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_LPModelabel = lv_label_create(ui_LPscreenContainer);
    lv_obj_set_width(ui_LPModelabel, LV_SIZE_CONTENT);   /// 1
    lv_obj_set_height(ui_LPModelabel, LV_SIZE_CONTENT);    /// 1
    lv_obj_set_x(ui_LPModelabel, 5);
    lv_obj_set_y(ui_LPModelabel, 141);
    lv_obj_set_align(ui_LPModelabel, LV_ALIGN_CENTER);
    lv_label_set_text(ui_LPModelabel, "ECO");
    lv_obj_set_style_text_color(ui_LPModelabel, lv_color_hex(0xFDFCFC), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_opa(ui_LPModelabel, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(ui_LPModelabel, &ui_font_sans32, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_changescreenLP = lv_button_create(ui_LPScreen);
    lv_obj_set_width(ui_changescreenLP, 453);
    lv_obj_set_height(ui_changescreenLP, 451);
    lv_obj_set_x(ui_changescreenLP, -15);
    lv_obj_set_y(ui_changescreenLP, 1);
    lv_obj_set_align(ui_changescreenLP, LV_ALIGN_CENTER);
    lv_obj_add_flag(ui_changescreenLP, LV_OBJ_FLAG_SCROLL_ON_FOCUS);     /// Flags
    lv_obj_remove_flag(ui_changescreenLP, LV_OBJ_FLAG_SCROLLABLE);      /// Flags
    lv_obj_set_style_bg_color(ui_changescreenLP, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(ui_changescreenLP, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_shadow_color(ui_changescreenLP, lv_color_hex(0x000000), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_shadow_opa(ui_changescreenLP, 0, LV_PART_MAIN | LV_STATE_DEFAULT);

    lv_obj_add_event_cb(ui_changescreenLP, ui_event_changescreenLP, LV_EVENT_ALL, NULL);

}

void ui_LPScreen_screen_destroy(void)
{
    if(ui_LPScreen) lv_obj_del(ui_LPScreen);

    // NULL screen variables
    ui_LPScreen = NULL;
    ui_LPscreenContainer = NULL;
    ui_MainTemptextLP = NULL;
    ui_DegreeLblLP = NULL;
    ui_TimeHLP = NULL;
    ui_DateLP = NULL;
    ui_TimeMLP = NULL;
    ui_TimeSLP = NULL;
    ui_LPModelabel = NULL;
    ui_changescreenLP = NULL;

}
